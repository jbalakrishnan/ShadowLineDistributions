...
def padic_height(self, p, prec=20, sigma=None, check_hypotheses=True):
    ...
    -  ``p`` - prime >= 3 for which the curve has
       semi-stable reduction
    ...
    if p != 3:
        ...
        R = rings.pAdicField(p,adjusted_prec)
...
def padic_sigma(self, p, N=20, E2=None, check=False, check_hypotheses=True):
    ...
    -  ``p`` - prime >= 3 for which the curve has good
       ordinary reduction
    ...
    Tate); case of p = 3 is in "On 3-adic heights on elliptic curves"
    (Jennifer Balakrishnan, JNT, 2016)
    ...
    - Jennifer Balakrishnan (2014-01): case of p = 3
    ...
    if E2 is None:
        ...
        if p == 3:
            E2 = self.padic_E2(p, N, check_hypotheses=False)
...
def padic_sigma_truncated(self, p, N=20, lamb=0, E2=None, check_hypotheses=True):
    ...
    via point multiplication" (David Harvey, still draft form); case
    of p = 3 is in "On 3-adic heights on elliptic curves"
    (Jennifer Balakrishnan, JNT, 2016)
    ...
    - Jennifer Balakrishnan (2014-01): added case of p = 3
...
def padic_E2(self, p, prec=20, check=False, check_hypotheses=True, algorithm="auto"):
    ...
    if p != 3:
        ...
        frob_p = self.matrix_of_frobenius(p, prec, check, check_hypotheses, algorithm).change_ring(Integers(p**(prec+3)))
    ...
    if p != 3:
        ...
        output_ring =rings.pAdicField(p,prec+2)
        alpha, delta, beta, eps = frob_p.list()
        alpha = output_ring(alpha)
        delta = output_ring(delta)
        beta = output_ring(beta)
        eps = output_ring(eps)
        fH, gH = self.hyperelliptic_polynomials()
        f = fH + (gH/2)**2
        if len(f.list()) == 4:
            a6, a4, a2, _ = f.list()
        elif len(f.list()) == 3:
        ...
        M00 = alpha-beta*A/3
        M01 = delta+alpha*A/3-(eps+beta*A/3)*A/3
        M10 = beta
        M11 = eps + beta*A/3
        M = matrix.matrix(output_ring,2,2,[M00,M01,M10,M11])
        if M11.valuation() > 1:
            self._anom=0
            output_ring = rings.pAdicField(p, prec - 1)
            self._E2_change_val = 1
        elif M11.valuation() == 0:
            self._anom = 0
            output_ring = rings.pAdicField(p, prec)
        elif M11.valuation() == 1:
            output_ring = rings.pAdicField(p,prec - (M01.valuation() + M10.valuation()))
        frob_p_n = M**prec
        E2_of_X = -12* frob_p_n[0,1]/frob_p_n[1,1]
        fudge_factor = (disc/self.discriminant()).nth_root(6)
